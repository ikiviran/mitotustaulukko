...
export function DisplayGroupTable() {
    const dispatch = useAppDispatch();
    const dgDict = useAppSelector((state) => state.data.dg);
    const [, setRerender] = useState(0); // hack to force rerendering
    ...
    const highlightedDgIds = dgs.filter(isRecentlyUpdated).map((dg) => dg.id);

    // This ugly code is just used to remove 'highlight' classes after 3 seconds
    useEffect(() => {
        if (highlightedDgIds.length > 0) {
            const interval = setInterval(() => {
                setRerender((prev) => prev + 1);
            }, 3000);
            return () => clearInterval(interval);
        }
    }, [highlightedDgIds.length]);
                ...
                <Table>
                    ...
                    <TableBody>
                        {dgs.map((dg) => (
                            <TableRow
                                key={dg.id}
                                className={highlightedDgIds.includes(dg.id) ? "highlight" : ""}
                            >
...
function CreateModal({ dgs, onClose }: CreateModalProps) {
    ...
    const createDg = async () => {
        const dg = await trpc.dg.create.mutate({
            code,
            name,
            level: Number(level),
            parent_id: parentId,
...
function EditModal({ dgs, dg, onClose }: EditModalProps) {
    ...
    const [level, setLevel] = useState<string>(String(dg.level));
        ...
        <Dialog open={true} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogActions>
                <Stack direction="row" justifyContent="space-between" width="100%">
                    <Button variant="text" color="error" onClick={deleteDg}>
                        Delete
                    </Button>
